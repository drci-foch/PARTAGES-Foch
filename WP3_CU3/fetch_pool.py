"""
Étape 1/2 — Récupération du pool de métadonnées CU3
Hôpital Foch / Projet PARTAGES

Lance une seule fois pour constituer pool_metadata.csv.
extract_cu3.py lit ce fichier sans retoucher la base.
...
def build_meta_query(config: CU3Config) -> str:
...
def fetch_pool(config: CU3Config = None):
    if config is None:
